...
parseInstruction str
    | str == "do()" = Just Do
    | str == "don't()" = Just Dont
    | "mul(" `isPrefixOf` str = case str =~ ("mul\\(([0-9]+),([0-9]+)\\)" :: String) :: (String, String, String, [String]) of
...
splitInstructions str = getAllTextMatches (str =~ instructionRegex)
  where
    instructionRegex = "(mul\\([0-9]+,[0-9]+\\)|do\\(\\)|don't\\(\\))" :: String
